(function($, $window, $document, undefined) {
    ...
    function setUpPicker(plugin, options) {
        ...
        plugin.inputBoxes = [];
        for(i in pointNames) {
            var input = makeInput(plugin.options.inputs[pointNames[i]], pointNames[i], plugin.name);
    ...
    function makeInput(opts, type, name) {
        return $('<input type="number"' +
            'id="' + (typeof opts.id === 'string' ? opts.id : '') +
            '" name="' + name + (typeof opts.name === 'string' ? opts.name : '') +
    ...
    function setupPickerLabels($wrapper, options) {
        ...
        $wrapper.prepend($('<div class="triangle-picker-top-label-wrapper"><label class="topMiddleLabel triangle-picker-label">	&#8226; </label></div>'));
        var bottomContainer = $('<div class="triangle-picker-bottom-label-wrapper"></div>');
        bottomContainer.append($('<label class="bottomLeft triangle-picker-label"> 	&#8226;	&#8226;	&#8226; </label>'));
        bottomContainer.append($('<label class="bottomRight triangle-picker-label">	&#8226;	&#8226;</label>'));
